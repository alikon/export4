...
class PlgExportCategory extends CMSPlugin
{
	 ...
	 * @since  3.5
	 */
	protected $serverUrl = 'http://localhost/sit/api/index.php/v1/content/categories';
	...
	public function onAjaxCategory()
	{
		...
		$this->serverUrl = $this->params->get('url', 'http://localhost/sit/api/index.php/v1/content/categories');
		...
		//JModelLegacy::getInstance('Category', 'CategoriesModel');
		...
		if ($response->code !== 200)
		{
			...
			$this->app->redirect(JRoute::_('index.php?option=com_categories&view=category&layout=edit&id=' . $id, false), $response->code . ' - ' . $data->errors[0]->title, 'error');
		}

		$this->app->redirect(JRoute::_('index.php?option=com_categories&view=category&layout=edit&id=' . $id, false), 'Exported', 'success');
